...
name: research-deep
description: >
  Read research outline and launch independent web-search agents for each item
  in parallel for deep research. Use after /deep-research:research has generated
  an outline. Supports resume from partial completion.
allowed-tools: Bash, Read, Write, Glob, WebSearch, Agent
---
...
## Web Search Module Path
Strategy modules are located at: ${{CLAUDE_SKILL_DIR}}/web-search-modules/
You MUST load the relevant module(s) from this path before executing any WebSearch.
...
python ${{CLAUDE_SKILL_DIR}}/../research/validate_json.py -f {fields_path} -j {output_path}
...
## Validation
After completing JSON output, run validation script to ensure complete field coverage:
python ${CLAUDE_SKILL_DIR}/../research/validate_json.py -f /home/weizhena/AIcoding/aicoding-history/fields.yaml -j /home/weizhena/AIcoding/aicoding-history/results/GitHub_Copilot.json
Task is complete only after validation passes.
```
